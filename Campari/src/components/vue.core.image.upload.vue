        ...
        <div class="g-crop-image-box">
          <div class="g-crop-image-principal">
            ...
            <div class="select-recorte" v-on:touchstart.self="drag" v-on:mousedown.self="drag" style="width:100px;height:100px;">
              ...
              <div class="g-resize" v-on:touchstart.self="resize" v-on:mousedown.self="resize" v-if="canResize"></div>
            </div>
          </div>
        </div>
      </div>
...
//              btn.disabled = true;
          ...
          __toCanvas() {
              ...
              let cw = this.data['toCropImgW'],
                  ch = this.data['toCropImgH'];
              if (!this.maxHeight && !this.maxWidth) {
                  //default
              } else if (this.maxHeight && !this.maxWidth) {
                  if (this.data['toCropImgH'] > this.maxHeight) {
                      ch = this.maxHeight;
                      cw = ch * this.data['toCropImgW'] / this.data['toCropImgH'];
                  }
              } else if (this.maxWidth && !this.maxHeight) {
                  if (this.data['toCropImgW'] > this.maxWidth) {
                      cw = this.maxWidth;
                      ch = cw * this.data['toCropImgH'] / this.data['coCropImgW'];
                  }
              } else if (this.maxWidth && this.maxHeight) {
                  let r = this.data['toCropImgW'] / this.data['toCropImgH'],
                      dr = this.maxWidth / this.maxHeight;
                  if (r < dr && this.data['toCropImgH'] > this.maxHeight) {
                      ch = this.maxHeight;
                      cw = r * ch;
                  } else if (r < dr && this.data['toCropImgH'] <= this.maxHeight) {
                      //default
                  } else if (r >= dr && this.data['toCropImgW'] > this.maxWidth) {
                      cw = this.maxWidth;
                      ch = cw / r;
                  } else if (r >= dr && this.data['toCropImgW'] >= this.maxWidth) {
                      //default
              ...
              context.drawImage(
                  this.image.img,
                  this.data['toCropImgX'], this.data['toCropImgY'],
                  this.data['toCropImgW'], this.data['toCropImgH'],
                  0, 0, canvas.width, canvas.height
              );
              this.base64 = canvas.toDataURL();
              this. __dispatch('transcodinged', this.base64);
              this.hasImage = false;
          },
