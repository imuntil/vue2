...
export class ImageResize {
  ...
  constructor(quill, options = {}) {
    ...
    this.body = document.querySelector('.quill-content')
  }
  ...
  show(img) {
    ...
    const bRect = this.body.getBoundingClientRect()
    this.positionBoxes(rect, bRect);
  ...
  showResizers() {
    ...
    document.addEventListener('click', this.checkImage, true)
  ...
  hideResizers() {
    ...
    document.removeEventListener('click', this.checkImage)
    this.quill.root.removeEventListener('input', this.checkImage);
    // reset user-select
    this.setUserSelect('');
    this.setCursor('');
    // remove boxes
    this.boxes.forEach(box => this.body.removeChild(box));
  ...
  positionBoxes(rect, br) {
    // set the top and left for each drag handle
    [
      { left: rect.left - 6 - br.left,              top: rect.top - 6 - br.top },               // top left
      { left: rect.left + rect.width - 6 - br.left, top: rect.top - 6 - br.top },               // top right
      { left: rect.left + rect.width - 6 - br.left, top: rect.top + rect.height - 6 - br.top }, // bottom right
      { left: rect.left - 6 - br.left,              top: rect.top + rect.height - 6 - br.top }, // bottom left
    ].forEach((pos, idx) => {
  ...
  hideSizeDisplay() {
    this.display && this.body.removeChild(this.display);
